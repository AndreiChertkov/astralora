...
def create_bb_layer_mzi(d_inp, d_out):
    ...
    decomposer = RealUnitaryDecomposerBatch(alg=decompose_alg) # TODO
    decomposer.v2m = vector_to_checkerboard # TODO
    decomposer.m2v = checkerboard_to_vector # TODO
    mesh_mode = "rectangle"
    crosstalk_filter_size = 5

    size_U = d_out * (d_out - 1) // 2
    size_S = min(d_out, d_inp)
    size_V = d_inp * (d_inp - 1) // 2
    ...
    def build_parameters() -> None:
        ...
        # phase_U = torch.Tensor(size_U)
        # phase_S = torch.Tensor(size_S)
        # phase_V = torch.Tensor(size_V)

        w = torch.randn(size_U + size_S + size_V)

        return w
    
    def create_usv_from_w(w):
        phase_U = w[:size_U]
        phase_S = w[size_U:size_U + size_S]
        phase_V = w[size_U + size_S:]

        return phase_U, phase_S, phase_V
    
    def build_weight_from_usv(U: Tensor, S: Tensor, V: Tensor) -> Tensor:
        ### differentiable feature is gauranteed
        if d_out == d_inp:
            weight = torch.mm(U, S.unsqueeze(1) * V)
        elif d_out > d_inp:
            weight = torch.mm(U[:, : d_inp], S.unsqueeze(1) * V)
        else:
            weight = torch.mm(U * S.unsqueeze(0), V[: d_out, :])
    ...
    def build_weight_from_phase(
        ...
        U = decomposer.reconstruct(delta_list_U, decomposer.v2m(phase_U))
        
        V = decomposer.reconstruct(delta_list_V, decomposer.v2m(phase_V))
        
        S = phase_S.cos()
        
        return build_weight_from_usv(U, S, V)
    
    def build_weight(w):
        phase_U, phase_S, phase_V = create_usv_from_w(w)
        weight = build_weight_from_phase(phase_U, phase_S, phase_V)

        return weight
    
    def bb(x, w):
        weight = build_weight(w).view(d_inp, d_out)
        return x @ weight
...
class RealUnitaryDecomposerBatch(object):
    ...
    def __init__(
        ...
        assert alg.lower() in {"reck", "clements", "francis"}, (
